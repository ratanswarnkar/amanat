...
function Get-OtpFromLog($logPath, $mobile) {
  ...
  $pattern = "mobile:\s*'$mobile'[\s\S]{0,200}?otp:\s*'(\d{6})'"
...
# Step 1: send OTP for normal user and capture logs
$server = Start-TestServer 'qa-step1-stdout.log' 'qa-step1-stderr.log'
try {
...
# Step 2: verify OTP + set PIN + login for normal user, capture auth logs only
$server = Start-TestServer 'qa-step2-stdout.log' 'qa-step2-stderr.log'
try {
...
# Step 3: send OTP for nominee user B and capture
$server = Start-TestServer 'qa-step3-stdout.log' 'qa-step3-stderr.log'
try {
...
# Step 4: verify OTP + set PIN for B, then create nominee link and send verification OTP
$server = Start-TestServer 'qa-step4-stdout.log' 'qa-step4-stderr.log'
try {
...
# Step 5: verify nominee link as approved
$server = Start-TestServer 'qa-step5-stdout.log' 'qa-step5-stderr.log'
try {
...
# Step 6: fresh server, normal /auth/login for nominee-role user B only
$server = Start-TestServer 'qa-step6-stdout.log' 'qa-step6-stderr.log'
try {
